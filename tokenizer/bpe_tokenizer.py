...
class RegexTokenizer:
    ...
    def save(self):
        ...
        *copied pretty much directly from karpathy/minbpe
        ...
        with open(merge_file, "w") as f:
            ...
            # Write the raw string pattern used
    ...
    def load(self):
        ...
        with open(merges_file, "r", encoding="utf-8") as f:
            # Read and compile pattern
            self.pattern = regex.compile(f.readline().strip())
    ...
    # Encodes token_id chunks, generated by `_encode_ordinary`
    def _encode_chunk(self, token_ids: list[int]) -> list[int]:
        while len(token_ids) >= 2:
            ...
            # Perform merge
            token_ids = self._merge(token_ids, min_pair, self.merges[min_pair])

        return token_ids

    # Least recently used (LRU) cache wrapper
    # Use tuples instead of list because tuples are immutable
    @lru_cache(maxsize=100_000)
    def _encode_chunk_cached(self, chunk: str) -> tuple[int, ...]:
        # Key is the chunk string, value is the tuple of utf-8 encoded integers
        return tuple(self._encode_chunk(list(chunk.encode("utf-8"))))
    ...
    def _encode_ordinary(self, text: str):
        ...
        for chunk in chunks:
            token_ids.extend(self._encode_chunk_cached(chunk))
